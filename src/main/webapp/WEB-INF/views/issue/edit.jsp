...
<%-- 재현 순서 줄바꿈 복원용 - detail.jsp/register.jsp와 동일 이유(오류상세 detail.jsp 주석 참고) --%>
<% pageContext.setAttribute("NEWLINE_CHARS", "\r\n"); %>
<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8">
<title>오류 수정 · #${issue.issueNumber} · QA로그</title>
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .field:first-of-type { border-top:none; padding-top:3px; }
  ...
  .existing-list { display:flex; flex-wrap:wrap; gap:8px; margin-bottom:8px; }
  .existing-item {
    display:flex; align-items:center; gap:6px; font-size:12px; background:var(--surface);
    border:1px solid var(--border); border-radius:8px; padding:6px 8px 6px 10px;
  }
  .existing-item .name { max-width:160px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
  .existing-item .remove { background:none; border:none; color:var(--danger); cursor:pointer; font-size:13px; }
...
<body>
  ...
  <main class="workspace">
    <aside class="project-panel" aria-label="프로젝트 목록">
      ...
      <nav class="project-list">
        <c:forEach var="projectItem" items="${projects}">
          ...
          <a class="project-item ${projectItem.id == issue.projectId ? 'is-selected' : ''}" href="${projectUrl}">
    ...
    <section class="issue-panel" aria-label="오류 수정">
      <div class="register-page">
        <div class="crumb"><c:out value="${issue.projectName}" /> / #${issue.issueNumber}</div>
        <h1 class="title">오류 수정</h1>

        <form class="panel" action="${ctx}/issues/${issue.id}/edit" method="post" enctype="multipart/form-data" id="editForm">
      <%-- 낙관적 잠금 대조용 원본 정밀도 값 - KoreanDateTime으로 가공하면 안 됨(detail.jsp와 동일 주의사항).
           이름을 IssueSaveRequestDTO.expectedUpdatedAt와 다르게 둔다 - 같은 이름이면 스프링이 이 필드를
           커맨드 객체(request)의 LocalDateTime 프로퍼티에도 자동 바인딩하려다 마이크로초 정밀도 문자열을
           못 파싱해서 400을 낸다(실제 재현해서 발견) --%>
      <input type="hidden" name="expectedUpdatedAtRaw" value="${issue.updatedAt}">
      <div id="deleteFields"></div>

      <div class="field">
        <label>제목</label>
        <input type="text" name="title" placeholder="제목 (선택)" value="${fn:escapeXml(issue.title)}">
      </div>
        ...
        <div>
          ...
          <select name="severity">
            <c:forEach var="opt" items="${severityOptions}">
              <option value="${opt.code}" ${opt.code == issue.severity ? 'selected' : ''}>${opt.label}</option>
            </c:forEach>
          </select>
        </div>
        <div>
          ...
          <select name="priority">
            <c:forEach var="opt" items="${priorityOptions}">
              <option value="${opt.code}" ${opt.code == issue.priority ? 'selected' : ''}>${opt.label}</option>
            </c:forEach>
          </select>
        </div>
      </div>
      ...
      <div class="field">
        <label>발생 위치</label>
        <input type="text" name="location" placeholder="화면·기능 (선택)" value="${fn:escapeXml(issue.location)}">
      </div>

      <div class="field">
        <label>URL 주소</label>
        <input type="text" name="locationUrl" placeholder="https:// (선택)" value="${fn:escapeXml(issue.locationUrl)}">
      </div>

      <div class="field row2">
        <div>
          <label>테스트 버전</label>
          <input type="text" name="testVersion" placeholder="예: v1.4.2-rc3" value="${fn:escapeXml(issue.testVersion)}">
        </div>
        <div>
          <label>테스트 환경</label>
          <input type="text" name="testEnvironment" placeholder="브라우저·기기·OS" value="${fn:escapeXml(issue.testEnvironment)}">
        </div>
      </div>

      <div class="field">
        <label>재현 순서</label>
        <textarea name="stepsToReproduce" placeholder="한 줄에 하나씩 입력하세요">${fn:escapeXml(issue.stepsToReproduce)}</textarea>
        <span class="hint">줄바꿈 기준으로 번호 목록으로 표시됩니다</span>
      </div>

      <div class="field">
        <label>기대 결과</label>
        <textarea name="expectedResult">${fn:escapeXml(issue.expectedResult)}</textarea>
        <div class="existing-list" id="existing-expected">
          <c:forEach var="a" items="${issue.attachments}">
            <c:if test="${a.context == 'expected_result'}">
              <span class="existing-item" data-attachment-id="${a.id}">
                <span class="name"><c:out value="${a.originalName}" /></span>
      ...
      <div class="field">
        <label>실제 결과</label>
        <textarea name="actualResult">${fn:escapeXml(issue.actualResult)}</textarea>
        <div class="existing-list" id="existing-actual">
          <c:forEach var="a" items="${issue.attachments}">
      ...
      <div class="field">
        <label>개선 방향</label>
        <textarea name="suggestedFix">${fn:escapeXml(issue.suggestedFix)}</textarea>
      </div>

      <div class="field">
        <label>첨부파일</label>
        <div class="existing-list" id="existing-general">
          <c:forEach var="a" items="${issue.attachments}">
      ...
      <div class="actions">
        <a class="btn" href="${ctx}/issues/${issue.id}">취소</a>
        <button type="submit" class="btn btn-primary" id="submitButton">수정 완료</button>
      </div>
        </form>
      </div>
    </section>
  ...
  <script>
    ...
    /** 기존 첨부의 × 버튼 - 화면에서 즉시 감추고, 제출 시 삭제 요청되도록 hidden 필드를 추가한다. */
    document.querySelectorAll('.existing-item .remove').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var item = btn.closest('.existing-item');
        var hidden = document.createElement('input');
        hidden.type = 'hidden';
        hidden.name = 'attachmentIdsToDelete';
        hidden.value = item.getAttribute('data-attachment-id');
        deleteFields.appendChild(hidden);
        item.remove();
      });
    });

    /** 첨부 드롭존 하나(일반/기대결과/실제결과 각각)를 독립적으로 동작하게 만든다. register.jsp와 동일 로직. */
